...
int main()
{
  ...
  {
  kxnet::GzipFile writer = kxnet::GzipFile::openForAppend(filename);
  if (writer.valid())
  {
  ...
  kxnet::GzipFile reader = kxnet::GzipFile::openForRead(filename);
  if (reader.valid())
  {
    char buf[256];
  ...
  {
  kxnet::GzipFile writer = kxnet::GzipFile::openForWriteExclusive(filename);
